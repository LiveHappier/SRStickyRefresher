...
class SecondViewController: UIViewController {
    ...
    @IBOutlet weak var tesLoader: UIView!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.tesLoader.layer.borderWidth = 2
        
        let color  = UIColor(red: 0/255, green: 209/255, blue: 192/255, alpha: 1)

        print("Je suis la position \(self.tesLoader.layer.position)")
        
        
        let test = CGPoint(x: self.tesLoader.frame.size.width / 2, y: self.tesLoader.frame.size.height / 2)
        let gradientRingLayer =
            WCGraintCircleLayer(bounds: self.tesLoader.bounds, position:test,fromColor: color, toColor:UIColor.white, linewidth:4.0, toValue:0)
        self.tesLoader.layer.addSublayer(gradientRingLayer)
        let duration = 3.0
